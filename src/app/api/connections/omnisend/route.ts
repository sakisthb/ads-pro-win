...
export async function POST(request: NextRequest) {
  ...
  try {
    // 3. Test the Omnisend connection via contacts endpoint
    const testRes = await safeFetch("https://api.omnisend.com/v3/contacts?limit=1", {
      headers: { "X-API-Key": apiKey },
    });
    if (!testRes.ok) {
      const detail = await testRes.text();
      console.error(
        `[api/connections/omnisend] Connection test failed (${testRes.status}): ${detail}`,
      );
      return json(
    ...
    // Validate the response is genuine Omnisend data
    const contactsData = (await testRes.json()) as unknown;
    if (!contactsData || typeof contactsData !== "object") {
      return json(
        { success: false, error: "Invalid response from Omnisend." },
        400,
      );
    }
    ...
    // 5. Upsert the AdAccount record for this brand + omnisend platform
    const existing = await prisma.adAccount.findFirst({
      where: { brandId, platform: "omnisend" },
    });
    ...
    console.error("[api/connections/omnisend] Error:", message);
