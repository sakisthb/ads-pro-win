...
All verification fixtures, temporary grants and ownership changes roll back, with explicit post-rollback checks. Role/default-grant preparation is confined to the disposable CI service. The vanilla smoke database separately applies the conditional migration without client fixture roles. Passing these checks verifies the CI database contract, not actual hosted Prisma credentials, Supabase Data API configuration, provider ingestion or production deployment.
