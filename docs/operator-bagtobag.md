...
Ads Pro Growth Center desk (engineering, 2026-09-16): Phase A stabilize is in the local working tree on `feat/growth-center-desk` (`resolveGrowthDeskOrigin` fail-closed outside development, deploy HTTPS guards, ADR two-hostnames, router + unit + live HTTP contract tests). Local dashboard proof (same day): BagToBag desk on `http://127.0.0.1:3000/dashboard` shows live SACOS counts (image issues / pending drafts / last accepted) via `GET /api/desk-summary` after Colima + Growth Center on `18806`; deep-link `http://127.0.0.1:18806/?site=bagtobag_com_gr`. Do **not** merge [PR #2](https://github.com/sakisthb/ads-pro-win/pull/2) until the operator says so. Commit/push of the uncommitted desk work waits on operator.
