...
- One existing target per immutable ten-minute preview: responsive Search ad final URL/copy update; positive keyword pause/final URL update; pause of a single campaign sitelink association; or a one-way Search campaign network repair that changes only `target_content_network` from `true` to `false`. Enabling Content Network, changing any other network setting, legacy call ads, keyword text/match changes, positive/negative creation, shared negative-list changes, shared asset edits, ad deletion, enable operations, campaign status, budgets and new campaigns are excluded.
...
- Execute requires visible approval of that exact preview and a separate acknowledgement that an enabled repaired target may continue or resume delivery/spend under existing budgets. For the network repair, the campaign can continue spending on Google Search after Content Network is disabled. It is not spend-neutral just because budgets are unchanged.
